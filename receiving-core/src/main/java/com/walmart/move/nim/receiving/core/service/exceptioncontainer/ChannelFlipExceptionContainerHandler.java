...
@Service
public class ChannelFlipExceptionContainerHandler extends ExceptionContainerHandler {
  @Override
  public void publishException(Container container) {
    ...
    if (tenantSpecificConfigReader.isFeatureFlagEnabled(
        ReceivingConstants.ENABLE_CF_SORTER_DIVERT)) {
      publishExceptionDivertToSorter(
          container.getTrackingId(), SorterExceptionReason.CHFLIP, container.getPublishTs());
    }
  }
}
