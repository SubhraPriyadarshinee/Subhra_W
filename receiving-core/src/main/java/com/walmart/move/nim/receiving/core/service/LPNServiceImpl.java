...
public class LPNServiceImpl implements LPNService {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(LPNServiceImpl.class);
  ...
  public CompletableFuture<Set<String>> retrieveLPN(int count, HttpHeaders headers) {
    ...
    String url = appConfig.getLpnBaseUrl() + ReceivingConstants.LPN25_GENERATOR_BY_COUNT + count;
    try {
      response =
