...
@Slf4j
@Service
public class FloorLoadedCaseProcessInstructionService extends DefaultBaseUpdateInstructionService {
    ...
    public InstructionResponse validateUserEnteredQty(
        ...
        if (eachCount > 0
            ...
            if (instruction4mDB.getReceivingMethod().equalsIgnoreCase(RxConstants.ReceivingTypes.PROBLEM_CASE)) {
                instruction4mDB.setReceivingMethod(RxConstants.ReceivingTypes.PROBLEM_CASE_RECEIVED_WITH_UNIT_SCANS);
            } else {
    ...
    public void processUpdateInstruction(UpdateInstructionRequest instructionUpdateRequestFromClient, DataHolder dataForUpdateInstruction, DeliveryDocument deliveryDocument4mDB, DeliveryDocumentLine deliveryDocumentLine4mDB, boolean setAuditQty, HttpHeaders httpHeaders) throws ReceivingException {
        ...
        DocumentLine documentLine =
                instructionUpdateRequestFromClient.getDeliveryDocumentLines().get(0);
        if (!documentLine.getQuantityUOM().equalsIgnoreCase(ReceivingConstants.EA)) {
        ...
        String instructionPackageInfo = dataForUpdateInstruction.getInstruction().getInstructionCreatedByPackageInfo();
        ...
        SsccScanResponse.Container containerScannedFromUI = updateInstructionServiceHelper.convertToGDMContainer(scannedDataMap);
        updateInstructionDataValidator.validateBarcodeNotAlreadyScanned(instructionPack, containerScannedFromUI);
        if (documentLine.getQuantityUOM().equalsIgnoreCase(ReceivingConstants.EA)) {
            updateInstructionDataValidator.validateScannedUnitBelongsToCasev2(dataForUpdateInstruction.getInstruction(), dataForUpdateInstruction.getGdmResponseForScannedData());
        }
        ...
        if (documentLine.getQuantityUOM().equalsIgnoreCase(ReceivingConstants.EA)) {
            ...
            quantityToBeReceivedInEaches =
                    ReceivingUtils.conversionToEaches(
                            dataForUpdateInstruction.getGdmResponseForScannedData().getUnitCount().intValue(),
                            ReceivingConstants.Uom.WHPK,
                            documentLine.getVnpkQty(),
                            documentLine.getWhpkQty());
            ...
            additionalInfo = selectedSerialInfoListEntry.getKey();
            log.debug(
                    "EpcisEnabled flow:  receiving  :: quantityToBeReceivedInEaches {} :: quantityToBeReceivedInVNPK {} :: additionalInfo :: {}"
                            + " :: gdmResponseForScannedData {} :: ",
                    quantityToBeReceivedInEaches,
                    quantityToBeReceivedInVNPK,
                    additionalInfo,
