...
public class ProblemPalletCreateInstructionServiceTest {
    ...
    @InjectMocks private ProblemPalletCreateInstructionService problemPalletCreateInstructionService;
    ...
    public void initMocks() {
        ...
        ReflectionTestUtils.setField(problemPalletCreateInstructionService, "gson", gson);
    ...
    public void testServeInstruction() throws ReceivingException, IOException {
        ...
        ssccScanResponse.getContainers().get(0).setParentId("12");
        ...
        for(SsccScanResponse.Container c : ssccScanResponse.getContainers()) {
            c.setHints(Collections.singletonList("UNIT_ITEM"));
            c.setSscc(null);
        ...
        request.setSscc("00200109395464720439");
        request.setDeliveryNumber("1234");
        request.setReceivingType("SSC");;
        ...
        scannedData1.setValue("00200109395464720439");
        scannedData1.setApplicationIdentifier("00");



        request.setScannedDataList(Arrays.asList(scannedData1));
        ...
        fitProblemTagResponse.getIssue().setUom("PH");
        fitProblemTagResponse.getResolutions().get(0).setState("OPEN");
        fitProblemTagResponse.getResolutions().get(0).setQuantity(1);
        ...
        Mockito.when(problemReceivingServiceHelper.
                getProjectedReceivedQtyInEaches(any(FitProblemTagResponse.class), any(InstructionRequest.class), anyList(), any())).thenReturn(1000);
        InstructionResponse response = problemPalletCreateInstructionService.serveInstruction(request, dataHolder, MockHttpHeaders.getHeaders());
        Assert.assertNotNull(response);
        Assert.assertEquals(response.getDeliveryDocuments().get(0).
                getDeliveryDocumentLines().get(0).getAdditionalInfo().getAttpQtyInEaches(), 576);
    }

    @Test
    public void testCalculateQuantitiesAndPersistIns() {
    }
