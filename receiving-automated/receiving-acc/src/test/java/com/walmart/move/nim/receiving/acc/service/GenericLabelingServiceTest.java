...
public class GenericLabelingServiceTest extends ReceivingTestBase {
  @InjectMocks private GenericLabelingService genericLabelingService;

  @Mock private AppConfig appConfig;

  private DeliveryDetails deliveryDetails;
  @Mock private ACCManagedConfig accManagedConfig;
  @Mock private PrintableUtils accPrintableUtils;
  ...
  public void testGenerateLabel() {
    LabelData labelData = genericLabelingService.generateLabel(null, 0);
    assertNull(labelData);
  }

  @Test
  public void testSend() {
    String sendResponse = genericLabelingService.send(null);
    assertNull(sendResponse);
  }

  @Test
  public void testPersistLabel() {
    genericLabelingService.persistLabel(null);
    assert (true);
  }

  @Test
  public void testZPLTemplateLabelData() {
    ...
    doReturn(getZplData()).when(accPrintableUtils).getLabelData(any(), anyString(), anyString());
    doReturn("1").when(labelingHelperService).getPOCode(anyString());
    String labelResponse =
        genericLabelingService.getFormattedLabelData(
    ...
    assert (labelResponse.contains("Store Friendly Shipping"));
  }
