...
public class EndGameOsdrServiceTest extends ReceivingTestBase {

  @Mock ReceiptService receiptService;
  @InjectMocks EndGameOsdrService endGameOsdrService;
  ...
  @Test
  public void testGetOsdrDetailsByDeliveryNumber() {
    ...
    for (OsdrPo osdrPo : osdrSummary.getSummary()) {
      ...
      assertEquals(
          osdrPo.getRcvdQty(), Integer.valueOf(4), "Osdr summary po received qty not matched");

      assertNull(osdrPo.getOverage());
      assertNull(osdrPo.getShortage());

      assertEquals(
          osdrPo.getDamage().getQuantity(),
          Integer.valueOf(0),
          "Osdr summary po damage qty not matched");
      assertEquals(
          osdrPo.getDamage().getCode(),
          OSDRCode.D53.getCode(),
          "Osdr summary po damage code not matched");
      assertEquals(
          osdrPo.getDamage().getClaimType(),
          ReceivingConstants.VDM_CLAIM_TYPE,
          "Osdr summary po damage claim type not matched");
      assertEquals(
      ...
      assertEquals(
          osdrPo.getDamage().getUom(),
          ReceivingConstants.Uom.VNPK,
          "Osdr summary po damage qty uom not matched");
      assertEquals(
      ...
      for (OsdrPoLine osdrPoLine : osdrPo.getLines()) {
        ...
        /*
        purchase reference number - 9763140005 and line 1 will have overage
        */
        ...
        */
        ...
        assertEquals(
            osdrPoLine.getDamage().getQuantity(),
            Integer.valueOf(0),
            "Osdr summary po line damage qty not matched");
        assertEquals(
            osdrPo.getDamage().getCode(),
            OSDRCode.D53.getCode(),
            "Osdr summary po damage code not matched");
        ...
        assertEquals(
            osdrPoLine.getDamage().getUom(),
            ReceivingConstants.Uom.VNPK,
            "Osdr summary po line damage qty uom not matched");
        assertEquals(
  ...
  @Test
  public void testGeneratePoReceipt() {
    String poNumber = "1708069842";
    when(gdmRestApiClient.getPurchaseOrder(anyString()))
        .thenReturn(EndGameUtilsTestCase.getPurchaseOrder());
    List<Receipt> receipts = getReceiptsForOSDRDetails();
    receipts.get(0).setPurchaseReferenceNumber(poNumber);

    PoReceipt poReceipt = endGameOsdrService.generatePoReceipt(poNumber, receipts);
    assertEquals(poReceipt.getEventPayload().getPoNumber(), poNumber);
  }
}
